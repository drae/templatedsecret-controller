...
func TestMapReference(t *testing.T) {
	...
	mapping := expansion.MappingFuncFor(declaredEnv, serviceEnv)

	for _, env := range envs {
		declaredEnv[env.Name] = expansion.Expand(env.Value, mapping)
	}
...
func TestMapping(t *testing.T) {
	...
	mapping := expansion.MappingFuncFor(context)
...
func TestMappingDual(t *testing.T) {
	...
	mapping := expansion.MappingFuncFor(context, context2)
...
func doExpansionTest(t *testing.T, mapping func(string) string) {
	...
	for _, tc := range cases {
		expanded := expansion.Expand(tc.input, mapping)
